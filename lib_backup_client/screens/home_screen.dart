...
class _HomeScreenState extends State<HomeScreen> {
  ...
  List<String> get _categories => context
            ...
            Expanded(
              child: CustomScrollView(
                ...
                slivers: [
                  ...
                  SliverToBoxAdapter(
                    child: _buildSectionHeader('Populaires près de vous'),
                  ),
                  SliverToBoxAdapter(child: _buildPopularRestaurants()),
                  SliverToBoxAdapter(
                    child: _buildSectionHeader('Offres du jour'),
                  ),
                  SliverToBoxAdapter(child: _buildDailyOffers()),
                  SliverToBoxAdapter(
                    child: _buildSectionHeader('Rapides (moins de 20 min)'),
                  ),
                  SliverToBoxAdapter(child: _buildFastRestaurants()),
                  SliverToBoxAdapter(child: _buildRewardsCard()),
                  SliverToBoxAdapter(
                    child: _buildSectionHeader('Épicerie fraîche'),
                  ),
                  ...
                  SliverToBoxAdapter(
                    child: _buildSectionHeader('À retirer gratuitement'),
                  ),
                  SliverToBoxAdapter(child: _buildFreePickupSection()),
                  SliverToBoxAdapter(
                    child: _buildSectionHeader('Dernières actualités'),
                  ),
          ...
          GestureDetector(
            onTap: () {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('Changement d\'adresse disponible'),
    ...
    return SizedBox(
      ...
      child: ListView(
        ...
        children: items
            .map(
              (item) =>
                  _buildMiniCard(item['emoji']!, item['name']!, item['price']!),
            )
            .toList(),
      ),
    );
    ...
    final items = [
      {
        ...
        'resto': 'chez Burger Palace',
      },
      {
        'emoji': '\u{1F35F}',
        'name': 'Frites offertes',
        'resto': 'chez Pizza Roma',
      },
    ];
  ...
  Widget _buildNewsSection() {
    final news = [
      {
        'title': 'Nouveau: Livraison en 15min à Agadir',
        'color': const Color(0xFF3D2C8D),
      },
      {
        'title': 'Top restaurants cette semaine',
        'color': const Color(0xFFE65100),
      },
      {
        'title': 'DeliVip Pass: 1 mois gratuit',
        'color': const Color(0xFF00BFA5),
      },
    ];
