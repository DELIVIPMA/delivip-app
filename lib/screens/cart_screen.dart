...
//  ✅ No global app bottom nav | ✅ Minimalist pill bottom bar
//  ✅ Supplements visible | ✅ Real images
// ═══════════════════════════════════════════════════════════════════
...
class _CartScreenState extends State<CartScreen> {
    ...
    return Scaffold(
      ...
      body: Consumer<CartProvider>(
        builder: (context, cart, _) {
          if (cart.isEmpty || cart.restaurantId.isEmpty) {
      ...
      // ═══ PILL BOTTOM BAR ═══ (exactement comme restaurant_menu_screen)
      bottomNavigationBar: _buildPillBar(),
    );
  }
  ...
  // ═══════════════════════════════════════════════════════
  //  PILL BAR — Uniquement celle-ci, pas de nav globale
  // ═══════════════════════════════════════════════════════
  ...
  Widget _buildPillBar() {
    return Consumer<CartProvider>(
      builder: (context, cart, _) {
        final hasContent =
            cart.items.isNotEmpty && cart.restaurantId.isNotEmpty;
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  ...
                  // Valider button (simple + center-screen animation overlay)
                  if (hasContent)
                    _CheckoutButton(
                      onCheckout: () => _onCheckout(context, cart),
                    )
                  else
                    GestureDetector(
                      onTap: () {
  ...
  void _onCheckout(BuildContext context, CartProvider cart) {
    ...
    // ─── Center-screen animation overlay (HUGE & OBVIOUS) ───
    showDialog(
      context: context,
            ...
            builder: (context, double value, child) {
              return SizedBox(
                ...
                child: Stack(
                  ...
                  children: [
                    // The Shopping Bag (Fixed at bottom)
                    Positioned(
                      bottom: 20,
                      child: Icon(
                        Icons.shopping_bag,
                    ...
                    // The Burger (Falling from top to inside the bag)
                    Positioned(
                      bottom: value, // Animates downwards
                      child: Icon(
                        Icons.fastfood,
                        size: 80,
    ...
    // Wait for the animation to finish, then route
    Future.delayed(const Duration(milliseconds: 1500), () {
      // 1. Close the animation dialog safely
...
class _CartItemCardState extends State<_CartItemCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final isOnlyOne =
        widget.cart.items.length == 1 && widget.cartItem.quantity == 1;
                ...
                child: SizedBox(
                  ...
                  child: Image.network(
                    ...
                    errorBuilder: (_, __, ___) => Container(
                      color: _teal.withValues(alpha: 0.08),
                      child: Center(
                        child: Icon(Icons.restaurant, color: _teal, size: 24),
                      ),
                    ),
                  ),
                ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: GoogleFonts.poppins(
                        fontSize: 11,
                        color: _textColor(opacity: 0.45),
                        height: 1.3,
                      ),
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                    ),
                    // ★ Supplements affichés ici ★
...
// ═══════════════════════════════════════════════════════════════════
//  CHECKOUT BUTTON — Simple "Valider" + arrow
//  (Center-screen animation overlay is triggered in _onCheckout)
// ═══════════════════════════════════════════════════════════════════
                ...
                child: SizedBox(
                  ...
                  child: Image.network(
                    ...
                    errorBuilder: (_, __, ___) => Container(
                      ...
                      child: Center(
                        child: Icon(Icons.fastfood, color: teal, size: 16),
                      ),
                    ),
                  ),
                ),
